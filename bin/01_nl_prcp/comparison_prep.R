...
knmi_prcp <- knmi_prcp[time >= min(gpm_prcp$time) & time <= max(gpm_prcp$time)]
rdr_prcp <- rdr_prcp[time >= min(gpm_prcp$time) & time <= max(gpm_prcp$time)]
...
knmi_prcp <- merge(gpm_cells, knmi_prcp)
...
rdr_prcp <- merge(gpm_cells, rdr_prcp)

gpm_knmi_prcp <- gpm_prcp[id %in% knmi_stations$nearest_cell] #Keep only gpm cells close to station locations
gpm_rdr_prcp <- gpm_prcp[id %in% rdr_cells$nearest_cell] #Keep only gpm cells close to radar cells
...
gpm_knmi_prcp <- merge(gpm_cells, gpm_knmi_prcp)
gpm_rdr_prcp <- merge(gpm_cells, gpm_rdr_prcp)

save(knmi_prcp, gpm_knmi_prcp, rdr_prcp, gpm_rdr_prcp, file = "./data/nl_prcp_main.rdata")
save(knmi_wet_days, gpm_wet_days, gpm_knmi_prcp, file = "./data/nl_prcp_wdays.rdata")
